...
static void video_image_display2(FFPlayer *ffp)
{
    ...
    if (vp->bmp) {
		...
		//av_log(ffp, AV_LOG_ERROR, "display pts:%"PRId64",pos:%"PRId64"\n",vp->pts,vp->pos);
...
retry:
        if (frame_queue_nb_remaining(&is->pictq) == 0) {
			...
			//for record
			if(opaque->is->ic->streams[0]->codecpar->width == 0 ||opaque->is->ic->streams[0]->codecpar->height == 0){
				opaque->is->ic->streams[0]->codecpar->width = vp->width;
				opaque->is->ic->streams[0]->codecpar->height = vp->height;
			}
...
static int tinterlace_filter_graph(FFPlayer *ffp, AVFilterGraph **graph, AVFilterContext **src,
                             AVFilterContext **sink, AVFrame *frame,char *name,char *args)
{
    ...
    avfitler = avfilter_get_by_name(name);
    if (!avfitler) {
        av_log(NULL, AV_LOG_ERROR, "Could not find the %s filter.\n",name);
        return AVERROR_FILTER_NOT_FOUND;
    }

    filter_ctx = avfilter_graph_alloc_filter(filter_graph, avfitler, name);
    if (!filter_ctx) {
        av_log(NULL, AV_LOG_ERROR,"Could not allocate the %s instance.\n",name);
        return AVERROR(ENOMEM);
    }
	
    err = avfilter_init_str(filter_ctx, args);
    if (err < 0) {
        av_log(NULL, AV_LOG_ERROR,"Could not initialize the %s filter.\n",name);
        return err;
    }
    ...
    err = avfilter_link(buffer_ctx, 0, filter_ctx, 0);
    if (err >= 0)
        err = avfilter_link(filter_ctx, 0, buffersink_ctx, 0);
    if (err < 0) {
...
static int ffplay_video_thread(void *arg)
{
    ...
    for (;;) {
		...
		bool needLut = false;
		if ((ffp->filterFileName != NULL && tempPath == NULL) ||
			(ffp->filterFileName != NULL && tempPath != NULL && strcmp(ffp->filterFileName,tempPath) != 0)){
			if (tingraph != NULL){
				avfilter_graph_free(&tingraph);
				tingraph = NULL;
			}
			needLut = true;
			av_log(NULL, AV_LOG_ERROR, "needLut FileName : %s\n",ffp->filterFileName);
		}else if (ffp->filterFileName == NULL && tempPath != NULL){
			if (tingraph != NULL){
				avfilter_graph_free(&tingraph);
				tingraph = NULL;
			}
			needLut = false;
			av_log(NULL, AV_LOG_ERROR, "no needLut \n");
		}
		tempPath = ffp->filterFileName;
		if((needLut /*|| ffp->video_play_type == AVPLAY_LIVE_INTERLACE || ffp->video_play_type == AVPLAY_PLAYBACK_INTERLACE*/)
			&& tingraph == NULL){
			char name[32] = {0};
			char args[256] = {0};
			if (needLut){
			...
			int iret = tinterlace_filter_graph(ffp, &tingraph,&b_in,&b_out,frame,name,args);
			if (iret != 0){
				if (tingraph != NULL){
					avfilter_graph_free(&tingraph);
					tingraph = NULL;
...
{	
    ...
    AVStream *out_stream ;
	int vfps = fps;
    av_log(ffp, AV_LOG_ERROR, "vfps = %d",fps);
    if (ffp->is_record!=AVRECORD_STATE_UNKNOWN) { 
    ...
    ffp->m_ofmt_ctx = NULL;
    ffp->m_ofmt = NULL;
    ffp->is_record = AVRECORD_STATE_INIT;
    ffp->record_error = 0;
    ...
    for (int i = 0; i < is->ic->nb_streams; i++) {
...
#if RECORD_ONLY_VIDEO
		 if(AVMEDIA_TYPE_VIDEO != in_stream->codec->codec_type)continue;
#endif
        out_stream = avformat_new_stream(ffp->m_ofmt_ctx, in_stream->codec->codec);
        if (!out_stream) {
            av_log(ffp, AV_LOG_ERROR, "Failed allocating output stream\n");
            goto end;
        }
        av_log(ffp, AV_LOG_ERROR, "i:%d,codec id:%d\n",i,in_stream->codecpar->codec_id);//AV_CODEC_ID_H264
        //av_log(ffp, AV_LOG_ERROR, "in stream time_base-- num:%d, den:%d\n",in_stream->time_base.num,in_stream->time_base.den);
        //av_log(ffp, AV_LOG_ERROR, "framerate time_base-- num:%d, den:%d\n",out_stream->codec->framerate.num,out_stream->codec->framerate.den);
        ...
        if(AVMEDIA_TYPE_VIDEO == in_stream->codec->codec_type){
			//av_log(ffp, AV_LOG_ERROR, "in  w:%d,h:%d \n",in_stream->codecpar->width,in_stream->codecpar->height);
			av_log(ffp, AV_LOG_ERROR, "video fps --in:%d, gop:%d\n",in_stream->codec->framerate.den,in_stream->codec->gop_size);
			out_stream->codecpar->width = in_stream->codecpar->width;
			out_stream->codecpar->height = in_stream->codecpar->height;
			out_stream->codec->width = in_stream->codecpar->width;
			out_stream->codec->height = in_stream->codecpar->height;
			out_stream->codec->time_base.den = in_stream->codec->time_base.den;
			out_stream->codec->time_base.num = in_stream->codec->time_base.num;
			in_stream->codec->framerate.den = vfps;
			in_stream->codec->framerate.num = 1;
			out_stream->codec->framerate.den = vfps;
			out_stream->codec->framerate.num = 1;	
			in_stream->codec->gop_size = vfps;
			out_stream->codecpar->extradata_size = 0;
			ffp->video_duration = 1000/vfps;
			//ffp->video_duration = 1000*1000/out_stream->time_base.den;
			av_dict_set_int(&ffp->format_opts, "video_track_timescale", 1000, 0);
		...
		#endif
			av_log(ffp, AV_LOG_ERROR, "audio duration:%"PRId64"\n",ffp->audio_duration);
		}
		//av_log(ffp, AV_LOG_ERROR, "out stream time_base-- num:%d, den:%d\n",out_stream->time_base.num,out_stream->time_base.den);
        out_stream->codec->codec_tag = 0;
		//ffp->m_ofmt_ctx->oformat->flags|=AVFMT_NODIMENSIONS;
   ...
   // av_log(ffp, AV_LOG_ERROR, "ffp->audio_duration:%"PRId64", ffp->video_duration:%"PRId64"\n",ffp->audio_duration,ffp->video_duration);
    av_log(ffp, AV_LOG_ERROR, "open record file %s\n",file_name);
    ...
    if (avformat_write_header(ffp->m_ofmt_ctx, &ffp->format_opts) < 0) {
	...
	ffp->video_num = 0;
	ffp->audio_num = 0;
	ffp->video_start_pts = 0;
	ffp->audio_start_pts = 0;
    av_log(ffp, AV_LOG_ERROR, "start record success! \n");
    return 0;
end:
    ffp->record_error ++;
    return -1;
...
static int ffp_get_spspps_len(unsigned char * data,int len){
	int i = 0;
	for(i=0;i<len;i++){
		if(data[i]==0&&data[i+1]==0&&data[i+2]==0&&data[i+3]==1&&((data[i+4] & 0x1F)==0x5 || (data[i+4] & 0x1F)==0x1))break;
	}
	return i;
}
...
int ffp_record_file(FFPlayer *ffp, AVPacket *packet)
{
	...
	if(isvideo){
		int nal_type = packet->data[4] & 0x1F; 
		if(nal_type == 0x7 || nal_type == 0x8 || nal_type == 0x5){
			packet->flags |= AV_PKT_FLAG_KEY;
			if(out_stream->codecpar->extradata_size == 0 && nal_type == 0x07){
				//av_log(ffp, AV_LOG_ERROR, "--dxp-- save sps,packet->size:%d \n",packet->size);
				out_stream->codecpar->extradata_size = ffp_get_spspps_len(packet->data,packet->size);
				//out_stream->codec->extradata_size = packet->size;
				out_stream->codecpar->extradata = malloc(out_stream->codecpar->extradata_size);
				if(out_stream->codecpar->extradata){
					av_log(ffp, AV_LOG_ERROR, "--dxp-- save sps , len:%d \n",out_stream->codecpar->extradata_size);
					memcpy(out_stream->codecpar->extradata,packet->data,out_stream->codecpar->extradata_size);
				}
			}
		}
	}else{
...
//modify by yuanzc for a/v sync
		if(isvideo){
			...
			//av_log(ffp, AV_LOG_ERROR, "audio pts:%"PRId64"\n",pkt->pts);
		}
...
{
	av_log(ffp, AV_LOG_ERROR, "stop record start! \n");
    assert(ffp);
    if (ffp->is_record == AVRECORD_STATE_RECORDING) {
        ffp->is_record = AVRECORD_STATE_UNINIT;
        pthread_mutex_lock(&ffp->record_mutex);
        ...
        av_log(ffp, AV_LOG_ERROR, "don't need stop record\n");
    }
    return 0;
}

int ffp_start_3dlut(FFPlayer *ffp,const char *filePath)
{
	int len = strlen(filePath);
	if (filePath == NULL || len <= 0)return -1;
	if (ffp->filterFileName != NULL && strcmp(ffp->filterFileName,filePath) == 0)return 1;
	if (ffp->filterFileName != NULL)free(ffp->filterFileName);
	
	ffp->filterFileName = (char *)mallocz(len + 1);
	sprintf(ffp->filterFileName,"%s",filePath);
	return 0;
}
int ffp_stop_3dlut(FFPlayer *ffp)
{
	if (ffp->filterFileName != NULL){
		free(ffp->filterFileName);
